...
class httpVerbs {
  ...
  private async viewAllRoles() {
    const result = await pool.query(
      `SELECT role.title AS job_title, role.id AS role_id, department.name AS department, role.salary AS Salary FROM role JOIN department ON role.department_id = department.id`
    );
    console.table(result.rows);
  }
  ...
  private async viewAllEmps() {
    const result =
      await pool.query(`SELECT employee.id AS Employee_id, employee.first_name, employee.last_name, role.title, role.salary,department.name AS Department_name, manager.first_name || ' '|| manager.last_name AS Managers
      FROM employee
